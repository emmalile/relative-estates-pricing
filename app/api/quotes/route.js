...
//
// A quote can be saved standalone (no approvalId) to sit in the repository
// as reusable reference pricing, or saved straight onto an approval by
// passing approvalId/projectId up front. Either way, /api/quotes/apply is
// what actually copies the numbers onto the approvals row.
//
// clientPrice mirrors Relative's documented formula — (cost + shipping) x
// markup — using the same shipping_ddp / markup_override fields as
// /api/approvals, so it's a like-for-like default. Treat it as a saved
// reference number, not the final source of truth for what's on the
// approval; recompute if your dashboard's pricing logic differs.
export async function POST(request) {
  ...
  // If this quote is going straight onto an approval and projectId wasn't
  // passed, look it up from the approval so filtering by projectId works
  // without an extra round trip on the caller's side.
