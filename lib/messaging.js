...
//
// A ten-digit number is assumed to be +1. That is an assumption about who
// this firm works with rather than a general truth; an international
// vendor has to be stored with its country code, which is why the vendor
// form's number is normalised at match time and never guessed at.
export function normalizePhone(raw) {
  ...
  // Anything else is ambiguous — a seven-digit local number, an extension
  // glued on, a typo. Refusing beats guessing wrong and messaging someone
  // else entirely.
  return null
}
