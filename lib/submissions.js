...
async function sendPricingCopy({ project, category, manufacturerName, pricingData, to }) {
  ...
  try {
    ...
    await resend.emails.send({
      from: 'Relative Estate <notifications@relativeestates.com>',
      to,
      ...
      html: `
        <div style="font-family: Georgia, serif; max-width: 560px; margin: 0 auto; padding: 40px 20px;">
          ...
          <div style="margin-top: 40px; padding-top: 24px; border-top: 1px solid #ede9e0; font-size: 11px; color: #b0aca4;">Relative Estate LLC · Kansas City, MO</div>
        </div>
      `,
    })
  } catch (emailError) {
  ...
  try {
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family: Georgia, serif; max-width: 560px; margin: 0 auto; padding: 40px 20px;">
          <div style="border-bottom: 1px solid #dedad2; padding-bottom: 24px; margin-bottom: 32px;">
            <div style="font-size: 11px; letter-spacing: 0.22em; text-transform: uppercase; color: #9a7a4a; margin-bottom: 8px;">Relative Estate — Material Pricing System</div>
