...
import { workbookToText } from './spreadsheet'
...
// Only the modern zipped format. The legacy binary .xls is a different format
// entirely, and the libraries that still read it are unmaintained — better to
// say "re-save it as .xlsx" than to carry that.
const SHEET_EXTS = ['xlsx', 'xlsm']
...
export function isExtractable(ext) {
  ...
  return PDF_EXTS.includes(e) || TEXT_EXTS.includes(e) || SHEET_EXTS.includes(e)
}

export function isSpreadsheet(ext) {
  return SHEET_EXTS.includes(normalizeExt(ext))
}

// Said in one place so the button, the route, and the error agree on which
// formats are handled and what to do about the ones that are not.
export function unsupportedFormatMessage(ext) {
  const e = normalizeExt(ext)
  if (e === 'xls') {
    return 'This is a legacy .xls file. Open it and save as .xlsx, then extract that.'
  }
  if (['numbers', 'pages', 'key'].includes(e)) {
    return `This is an Apple ${e === 'numbers' ? 'Numbers' : 'iWork'} file. Export it as .xlsx or PDF, then extract that.`
  }
  if (['doc', 'docx'].includes(e)) {
    return 'Word documents are not read yet. Save it as a PDF and extract that.'
  }
  return `Cannot read a .${e || 'file'} — extraction handles PDFs, spreadsheets (.xlsx) and CSVs.`
}
...
async function userContent({ fileName, ext, bytes, category }) {
  const instruction = {
    type: 'text',
    text:
      `Document: ${fileName}\n\n` +
  ...
  if (PDF_EXTS.includes(normalizeExt(ext))) {
    return [
      {
        type: 'document',
  ...
  if (isSpreadsheet(ext)) {
    const { text } = await workbookToText(bytes, fileName)
    return [
      {
        type: 'text',
        text:
          `<workbook name="${fileName}">\n` +
          'Each sheet is given as CSV. The first value on every line is that ' +
          "row's number in the sheet, not data — use it for sourceLocation, " +
          'together with the sheet name.\n\n' +
          `${text}\n</workbook>`,
      },
      instruction,
    ]
...
export async function extractItems({ categoryId, fileName, ext, bytes }) {
  ...
  const stream = client.messages.stream({
    ...
    messages: [
      { role: 'user', content: await userContent({ fileName, ext, bytes, category }) },
    ],
  })
