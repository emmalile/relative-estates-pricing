...
// ── Applications ──────────────────────────────────────────
// A location reads "ROOM — APPLICATION" ("FOYER 101 — Flooring"), built by
// buildScheduleItems from the room and area columns. Ten of those on one
// line answers a question nobody asked: what matters at a glance is that
// this stone is flooring, not which eleven rooms it goes in.
//
// Returns the distinct applications, in the order they first appear.
export function applicationsFor(item) {
  const seen = []
  ;(item?.locations || []).forEach(loc => {
    const parts = String(loc).split('—').map(p => p.trim()).filter(Boolean)
    // No dash means the whole string is a room with no stated application.
    const application = parts.length > 1 ? parts[parts.length - 1] : null
    if (application && !seen.includes(application)) seen.push(application)
  })
  return seen
}

// The rooms alone, for where the detail genuinely belongs.
export function roomsFor(item) {
  return (item?.locations || []).map(loc => {
    const parts = String(loc).split('—').map(p => p.trim()).filter(Boolean)
    return parts.length > 1 ? parts.slice(0, -1).join(' — ') : String(loc).trim()
  }).filter(Boolean)
}
