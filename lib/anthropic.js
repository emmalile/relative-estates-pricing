...
// The same diagnostic the Files page renders, in one place so the page, the
// error returned when a read is refused, and the server log cannot disagree.
...
// Written to the server log as well as returned to the caller. A diagnostic
// that only reaches the browser can only be read by whoever is holding the
// browser, which is no help when the person diagnosing it is somewhere else.
//
// Variable names only. Values are never read here and never logged.
export function logExtractionUnconfigured(where) {
  console.warn(`[extraction] not configured at ${where}:`, JSON.stringify(extractionDiagnostics()))
}
